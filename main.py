...
Routes:
  ...
  POST /roadmap/{id}/approve
  GET  /roadmap/{user_id}/active
  GET  /tasks/{user_id}/today
  POST /tasks/{task_id}/complete
  GET  /progress/{user_id}
  GET  /leaderboard/{group_id}
"""
...
from backend.db.models import (
    init_db, get_db, User, Roadmap, Task, DailyProgress,
    TaskStatus, TaskType
)
...
async def me(user: User = Depends(get_current_user)):
    return {
        ...
        "email":    user.email, "mode":   user.mode,
        "streak":   user.streak, "total_xp": user.total_xp,
...
async def roadmap_generate(
    ...
    roadmap_data = generate_roadmap(
        data.topic, data.goal, data.skill_level, data.duration_days
    )
    roadmap = Roadmap(
        user_id=user.id, topic=data.topic, goal=data.goal,
...
async def roadmap_active(
    ...
    if not roadmap:
        return {"roadmap": None}
    return {"roadmap": json.loads(roadmap.roadmap_json), "roadmap_id": roadmap.id,
            "topic": roadmap.topic, "duration_days": roadmap.duration_days}
...
async def tasks_today(
    ...
    # Get active roadmap
    r = await db.execute(
        select(Roadmap).where(Roadmap.user_id == user_id, Roadmap.approved == True)
        .order_by(Roadmap.created_at.desc())
    )
    roadmap = r.scalars().first()
    if not roadmap:
        return {"tasks": [], "day_number": 0}
    ...
    t = await db.execute(
        select(Task).where(Task.roadmap_id == roadmap.id, Task.day_number == days_since)
    )
    existing = t.scalars().all()
    if existing:
        return {"tasks": [_task_to_dict(tk) for tk in existing], "day_number": days_since}

    # Generate new tasks for today
    roadmap_data = json.loads(roadmap.roadmap_json)
    phase_name   = _get_current_phase(roadmap_data, days_since)
    raw_tasks    = generate_daily_tasks(
        roadmap.topic, phase_name, days_since, roadmap.skill_level
    )

    tasks = []
    for rt in raw_tasks:
        task = Task(
            roadmap_id=roadmap.id, user_id=user_id,
            day_number=days_since, title=rt.get("title", "Study task"),
            description=rt.get("description", ""), 
            task_type=TaskType(rt.get("task_type", "read")),
            xp_reward=rt.get("xp_reward", 10),
    ...
    return {"tasks": [_task_to_dict(t) for t in tasks], "day_number": days_since}

@app.post("/tasks/{task_id}/complete")
async def task_complete(
    task_id: str,
    data: TaskCompleteSchema,
    user: User = Depends(get_current_user),
    db:   AsyncSession = Depends(get_db)
):
...
async def get_progress(
    ...
    avg_score = sum(t.score for t in tasks if t.score) / max(completed, 1)
    ...
    return {
        ...
        "streak":          target_user.streak if target_user else 0,
        "total_xp":        target_user.total_xp if target_user else 0,
    }
...
async def get_leaderboard(
    ...
    return {
        "leaderboard": [
            {"name": m.name, "xp": m.total_xp,
             "streak": m.streak, "rank": i + 1}
...
def _get_current_phase(roadmap_data: dict, day_number: int) -> str:
    for phase in roadmap_data.get("phases", []):
        days_range = phase.get("days", "1-7")
        try:
